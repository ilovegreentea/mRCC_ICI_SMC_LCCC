...
## =========================
## INPUT (must already exist)
##  - PBMC_T : Seurat object (blood)
##  - T_cell : Seurat object (tissue/tumor)
## =========================
...
## PBMC origin-defining labels (as provided)
PBMC_PEX_LABEL   <- "CD8_pEx"
PBMC_TEMRA_LABEL <- "CD8_Temra"

## TCR key: match by CTnt (CTaa can be less stable for cross-compartment matching)
TCR_COL <- "CTnt"
stopifnot(TCR_COL %in% colnames(PBMC_T@meta.data))
stopifnot(TCR_COL %in% colnames(T_cell@meta.data))

## Tumor state column (used for coloring)
TUMOR_STATE_COL <- "T_NK_cell"
stopifnot(TUMOR_STATE_COL %in% colnames(T_cell@meta.data))

## Response / Timepoint columns
stopifnot(all(c("Response","Timepoint") %in% colnames(T_cell@meta.data)))
stopifnot("Merged" %in% colnames(PBMC_T@meta.data))

## =========================
## Fixed palette (as provided)
## =========================
tnk_palette <- c(
  "CD56-high_NK" = "#9BC8BE",
  "CD56-low_NK"  = "#F5F0B9",
...
## Optional: normalize long tumor-state labels to palette keys
normalize_tumor_state <- function(x) {
  dplyr::case_when(
...
## =========================
## Fisher fallback: if exact fails, use simulated p-value
##  - This does NOT change the contingency table; it only approximates the p-value.
## =========================
safe_fisher <- function(tab, B = 20000, workspace = 2e8) {
  tab <- as.matrix(tab)

  ## fisher requires at least 2x2
  if (nrow(tab) < 2 || ncol(tab) < 2) {
    return(list(p.value = NA_real_, method = "not_testable", B = NA_integer_))
  }

  ## Try exact first
  out <- tryCatch({
    ft <- fisher.test(tab, workspace = workspace)
    list(p.value = unname(ft$p.value), method = "exact", B = NA_integer_)
  }, error = function(e) {
    NULL
  })
  ...
  ## If exact fails, fall back to simulation
  ft2 <- suppressWarnings(fisher.test(tab, simulate.p.value = TRUE, B = B))
  list(p.value = unname(ft2$p.value), method = "simulate", B = B)
}

## =========================================================
## 1) Build clonotype sets in PBMC (CTnt) for pEx vs Temra
## =========================================================
pbmc_md <- PBMC_T@meta.data

pEx_clones <- pbmc_md %>%
  filter(!is.na(.data[[TCR_COL]]), .data[[TCR_COL]] != "") %>%
  filter(Merged == PBMC_PEX_LABEL) %>%
  distinct(clone = .data[[TCR_COL]]) %>%
  pull(clone)

temra_clones <- pbmc_md %>%
  filter(!is.na(.data[[TCR_COL]]), .data[[TCR_COL]] != "") %>%
  filter(Merged == PBMC_TEMRA_LABEL) %>%
  distinct(clone = .data[[TCR_COL]]) %>%
  pull(clone)

## Handle overlap: split into "Both" (important)
## - For pEx vs Temra origin comparisons, overlapping clones make origin ambiguous.
both_clones <- intersect(pEx_clones, temra_clones)

message("PBMC pEx clones: ", length(pEx_clones))
message("PBMC Temra clones: ", length(temra_clones))
message("PBMC overlap (Both): ", length(both_clones))

## =========================================================
## 2) Assign origin to each tumor cell (CTnt matching)
## =========================================================
tumor_md <- T_cell@meta.data %>%
  mutate(
    clone = .data[[TCR_COL]],
    Tumor_state = normalize_tumor_state(.data[[TUMOR_STATE_COL]]),
    Tissue_tp = as.character(Timepoint),
    Response = as.character(Response)
  ) %>%
  filter(!is.na(clone), clone != "") %>%
  mutate(
    origin = case_when(
      clone %in% both_clones ~ "Both",
      clone %in% pEx_clones ~ "pEx",
      clone %in% temra_clones ~ "Temra",
      TRUE ~ "Other"
    )
  )

## Analysis set: only tumor cells with origin pEx or Temra (exclude Both/Other)
tumor_matched <- tumor_md %>%
  filter(origin %in% c("pEx","Temra")) %>%
  filter(!is.na(Tumor_state), Tumor_state != "") %>%
  filter(!is.na(Response), Response != "") %>%
  filter(!is.na(Tissue_tp), Tissue_tp != "")

if (nrow(tumor_matched) == 0) stop("No tumor cells matched to pEx/Temra origin. Check CTnt matching/labels.")

write.table(
  tumor_matched %>% select(clone, origin, Tissue_tp, Response, Tumor_state),
  file = file.path(OUTDIR, "tumor_cells_matched.tsv"),
  sep = "\t", quote = FALSE, row.names = FALSE
)

## =========================================================
## 3) TEST #1: Overall Fisher on origin(pEx vs Temra) x Tumor_state (2xK)
## =========================================================
tab1 <- tumor_matched %>%
  count(origin, Tumor_state, name = "n") %>%
  xtabs(n ~ origin + Tumor_state, data = .)

ft1 <- safe_fisher(tab1)

test1_res <- tibble(
  test = "Overall: origin (pEx vs Temra) × Tumor_state",
  p_value = ft1$p.value,
  method = ft1$method,
  B = ft1$B,
  n_cells = sum(tab1)
...
write.table(test1_res, file.path(OUTDIR, "fisher_overall_origin_vs_tumorstate.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)

## =========================================================
## 4) TEST #2 & #3:
##  - Within origin == pEx: per tissue timepoint, Fisher on Response x Tumor_state
##  - Within origin == Temra: same
##  - BH-adjust p-values across timepoints within each origin
## =========================================================
fisher_by_origin_tp <- function(df_origin) {
  df_origin %>%
    group_by(origin, Tissue_tp) %>%
    group_modify(function(.x, .g) {
      tab <- .x %>%
        count(Response, Tumor_state, name = "n") %>%
        xtabs(n ~ Response + Tumor_state, data = .)
...
test23_res <- fisher_by_origin_tp(tumor_matched)

write.table(test23_res, file.path(OUTDIR, "fisher_by_origin_timepoint_response_vs_tumorstate.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)

## =========================================================
## 5) PLOT A: Overall comparison (TEST #1)
##  - x: origin, fill: Tumor_state, y: proportion
##  - show test type + p-value/method in the subtitle
## =========================================================
plot_overall <- tumor_matched %>%
  count(origin, Tumor_state, name = "n") %>%
  group_by(origin) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

p_overall <- ggplot(plot_overall, aes(x = origin, y = prop, fill = Tumor_state)) +
  geom_col(width = 0.8) +
  scale_fill_manual(values = tnk_palette, drop = FALSE) +
  theme_bw(base_size = 12) +
  labs(
    title = "Overall tumor-state distribution differs by PBMC origin (pEx vs Temra)",
    subtitle = paste0(
      "Test: Fisher exact on 2×K table [origin × tumor_state]. ",
      "p=", signif(test1_res$p_value, 3),
      " (method=", test1_res$method,
      ifelse(is.na(test1_res$B), "", paste0(", B=", test1_res$B)),
      ")."
    ),
    x = "PBMC origin (defined by PBMC Merged: CD8_pEx vs CD8_Temra)",
    y = "Proportion (within origin)",
    fill = "Tumor state (T_NK_cell)"
  ) +
  theme(legend.position = "bottom")

ggsave(file.path(OUTDIR, "PLOT_A_overall_origin_vs_tumorstate.pdf"),
       p_overall, width = 8.5, height = 4.5)
ggsave(file.path(OUTDIR, "PLOT_A_overall_origin_vs_tumorstate.png"),
       p_overall, width = 8.5, height = 4.5, dpi = 300)

print(p_overall)

## =========================================================
## 6) PLOT B: per-origin, per-timepoint Response comparison (TEST #2/#3)
##  - for each (origin, timepoint): Fisher on Response x Tumor_state (BH-adjust within origin)
## =========================================================
plot_tp <- tumor_matched %>%
  count(origin, Tissue_tp, Response, Tumor_state, name = "n") %>%
  group_by(origin, Tissue_tp, Response) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()
...
lab_tp <- test23_res %>%
  transmute(
    origin, Tissue_tp,
    label = paste0("Fisher (Response×state), BH p=", signif(p_adj_BH, 3),
                   ifelse(method == "simulate", paste0(" [sim B=", B, "]"), ""))
  )

p_tp2 <- ggplot(plot_tp, aes(x = Tissue_tp, y = prop, fill = Tumor_state)) +
  geom_col(width = 0.8) +
  facet_grid(origin ~ Response) +
  scale_fill_manual(values = tnk_palette, drop = FALSE) +
  theme_bw(base_size = 12) +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.18))) +
  labs(
    title = "Tumor states of PBMC-derived clonotypes (pEx vs Temra), stratified by response",
    subtitle = "Within each origin and tumor timepoint, Fisher tests whether tumor_state distribution differs between responses (BH-adjusted within origin).",
    x = "Tumor timepoint",
    y = "Proportion within (origin, timepoint, response)",
    fill = "Tumor state (T_NK_cell)"
  ) +
  theme(
    legend.position = "bottom",
  ...
  geom_text(
    data = lab_tp,
    aes(x = Tissue_tp, y = 1.08, label = label),
    inherit.aes = FALSE,
    size = 3.2
  )

ggsave(file.path(OUTDIR, "PLOT_B_origin_timepoint_response_fisher.pdf"),
       p_tp2, width = 10.5, height = 6.5)
ggsave(file.path(OUTDIR, "PLOT_B_origin_timepoint_response_fisher.png"),
       p_tp2, width = 10.5, height = 6.5, dpi = 300)
...
## =========================
## INPUT: PBMC_T and T_cell must exist
## =========================
...
stopifnot(all(c(TCR_COL, "Merged") %in% colnames(PBMC_T@meta.data)))
stopifnot(all(c(TCR_COL, TUMOR_STATE_COL, "Response", "Timepoint") %in% colnames(T_cell@meta.data)))
...
## If Fisher exact fails, fall back to simulated p-value (same table; approximate p-value only)
safe_fisher_p <- function(tab, workspace = 2e8, B = 20000) {
  tab <- as.matrix(tab)
  if (nrow(tab) < 2 || ncol(tab) < 2) return(list(p = NA_real_, method = "not_testable", B = NA_integer_))
  out <- tryCatch({
    ft <- fisher.test(tab, workspace = workspace)
    list(p = unname(ft$p.value), method = "exact", B = NA_integer_)
  }, error = function(e) NULL)
  if (!is.null(out)) return(out)
  ft2 <- suppressWarnings(fisher.test(tab, simulate.p.value = TRUE, B = B))
  list(p = unname(ft2$p.value), method = "simulate", B = B)
}

## =========================
## 1) Build clonotype sets in PBMC (CTnt) for pEx/Temra
## =========================
pbmc_md <- PBMC_T@meta.data
...
pEx_clones <- pbmc_md %>%
  filter(!is.na(.data[[TCR_COL]]), .data[[TCR_COL]] != "") %>%
  filter(Merged == PBMC_PEX_LABEL) %>%
  distinct(clone = .data[[TCR_COL]]) %>% pull(clone)
...
## =========================
## 2) Assign origin to tumor cells by CTnt matching
## =========================
tumor_md <- T_cell@meta.data %>%
  mutate(
    clone = .data[[TCR_COL]],
    Tumor_state = as.character(.data[[TUMOR_STATE_COL]]),
    Tissue_tp = as.character(Timepoint),
    Response = as.character(Response),
    origin = case_when(
      clone %in% both_clones ~ "Both",
  ...
  filter(!is.na(clone), clone != "") %>%
  filter(origin %in% c("pEx", "Temra")) %>%   # Exclude Both/Other
  filter(!is.na(Tumor_state), Tumor_state != "") %>%
  filter(!is.na(Response), Response != "") %>%
  filter(!is.na(Tissue_tp), Tissue_tp != "")

if (nrow(tumor_md) == 0) stop("No tumor cells matched to pEx/Temra origin.")

## =========================
## 3) Plot data: tumor_state proportions within (origin, timepoint, response)
## =========================
plot_df <- tumor_md %>%
  count(origin, Tissue_tp, Response, Tumor_state, name = "n") %>%
  group_by(origin, Tissue_tp, Response) %>%
...
## =========================
## 4) Fisher: per (origin, timepoint), test Response × Tumor_state
##    BH-adjust across timepoints within each origin
## =========================
fisher_res <- tumor_md %>%
  count(origin, Tissue_tp, Response, Tumor_state, name = "n") %>%
  group_by(origin, Tissue_tp) %>%
  summarise(
    p_value = {
      tab <- xtabs(n ~ Response + Tumor_state, data = cur_data())
      safe_fisher_p(tab)$p
    },
    method = {
      tab <- xtabs(n ~ Response + Tumor_state, data = cur_data())
      safe_fisher_p(tab)$method
    },
    B = {
      tab <- xtabs(n ~ Response + Tumor_state, data = cur_data())
      safe_fisher_p(tab)$B
  ...
  mutate(
    label = paste0(
      "Fisher(Response×state) BH p=",
      signif(p_adj_BH, 3),
      ifelse(method == "simulate", paste0(" [sim B=", B, "]"), "")
    )
  )

## Place one label per panel at the top
panel_cap <- plot_df %>%
  group_by(origin, Tissue_tp) %>%
  summarise(y = 1.06, .groups = "drop")

fisher_lab <- fisher_res %>%
  left_join(panel_cap, by = c("origin", "Tissue_tp"))

write.table(fisher_res, file.path(OUTDIR, "fisher_by_origin_timepoint_Response_vs_TumorState.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)

## =========================
## 5) Final plot
##   - facet: origin ~ Tissue_tp
##   - x: Response (side-by-side comparison within the same timepoint)
## =========================
plot_df <- plot_df %>%
  mutate(
...
p <- ggplot(plot_df, aes(x = Response, y = prop, fill = Tumor_state)) +
  geom_col(width = 0.8) +
  facet_grid(origin ~ Tissue_tp) +
  geom_text(
    data = fisher_lab,
    aes(x = 1.5, y = y, label = label),  # One label per panel
    inherit.aes = FALSE,
    size = 3.2
  ) +
  scale_fill_manual(values = tnk_palette, drop = FALSE) +
  ...
  theme_bw(base_size = 12) +
  labs(
    title = "Tumor states of PBMC-derived clonotypes (pEx vs Temra)",
    subtitle = "Within each (origin, tumor timepoint), Fisher tests whether tumor_state distribution differs by Response (BH-adjusted within origin).",
    x = "Response (within same tumor timepoint)",
    y = "Proportion within (origin, timepoint, response)",
    fill = "Tumor state (T_NK_cell)"
  ) +
  theme(
...
ggsave(file.path(OUTDIR, "TumorStates_PBMCorigin_facet_origin_timepoint_xResponse.pdf"),
       p, width = 11, height = 6.5)
ggsave(file.path(OUTDIR, "TumorStates_PBMCorigin_facet_origin_timepoint_xResponse.png"),
...
tnk_palette <- c(
  ...
  "Erythrocyte"         = "#CDE1C3" ,
  "NKT"           = "#E7DD6A",
  "Proliferative" = "#B96A3B",
  "Tgd"           = "#B0A7D6"
)

## -----------------------------
## Thresholds (edit only here)
## -----------------------------
min_sample_tcr_cells <- 500      # Per-sample TCR+ cell cutoff
hyper_freq_cut <- 0.002          # 0.2%
min_clone_cells_for_hyper <- 2   # (Optional) avoid sc artifacts; set to 1 if desired

prop_cut_for_bubbles <- 0.90     # Cumulative proportion to draw in each bubble panel
area_scale <- 1e9
npoints <- 20
...
## 1) meta
## -----------------------------
PBMC_T <- subset(PBMC_T, Timepoint == "T0")
meta <- PBMC_T@meta.data %>% rownames_to_column("barcode")
...
clone_sample <- meta_tcr %>%
  ...
  summarise(
    n_cells = n(),
    T_NK_cell_mode = names(sort(table(Merged), decreasing = TRUE))[1],
    .groups = "drop"
  ) %>%
  group_by(sample_id) %>%
...
## -----------------------------
## 6) Final plot: rows = expanded, cols = T_R_mode
## -----------------------------
p_bubble_TR <- ggplot(bubble_plot_df) +
  geom_polygon(aes(x, y, group = id, fill = T_NK_cell_mode),
               color = "white", alpha = 1, linewidth = 0.01) +
  scale_fill_manual(values = tnk_palette, drop = FALSE) +
  facet_grid(rows = vars(expanded), cols = vars(T_R_mode), drop = FALSE) +
  coord_fixed(expand = FALSE) +
  theme_void() +
  labs(fill = NULL) +
  theme(
...
## Keep only TCR+ cells (for confirmation/labeling)
meta_tcr <- meta %>%
  filter(!is.na(.data[[clone_id_col]]), .data[[clone_id_col]] != "") %>%
  filter(!is.na(sample_id), sample_id != "")
...
## -----------------------
PBMC_T$expanded <- NA_character_
PBMC_T$expanded[cell_expanded$barcode] <- cell_expanded$expanded
PBMC_T$expanded <- factor(PBMC_T$expanded, levels = c("Non-Expanded","Hyper-Expanded"))

PBMC_T$expanded <- NULL

PBMC_T@meta.data <- PBMC_T@meta.data %>%
  tibble::rownames_to_column("barcode") %>%
  left_join(
    cell_expanded %>% select(barcode, expanded),
    by = "barcode"
  ) %>%
  mutate(expanded = factor(expanded, levels = c("Non-Expanded","Hyper-Expanded"))) %>%
...
NA_PBMC_T <- subset(PBMC_T, expanded != "NA")

p <- dittoSeq::dittoBarPlot(
  object = NA_PBMC_T,
  var = "Merged",
  group.by = "expanded"
) + scale_fill_manual(values = tnk_palette, drop = FALSE)
...
NA_PBMC_T@meta.data %>% nrow()
dittoSeq::dittoBarPlot(NA_PBMC_T, var = "Merged", group.by = "expanded")

library(dplyr)

meta <- PBMC_T@meta.data
nkt_label <- "NKT"  # Change if needed

nkt_top <- meta %>%
  filter(Merged == nkt_label, !is.na(CTaa), CTaa != "") %>%
...
top_clone <- nkt_top %>% slice(1) %>% pull(CTaa)

meta %>%
  filter(CTaa == top_clone) %>%
  count(Merged, sort = TRUE) %>%
  mutate(freq = n / sum(n))
